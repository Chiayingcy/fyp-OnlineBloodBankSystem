...
<head>
    ...
    <title>Hospitals Update Profile</title>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method = "POST" action="{{ route ('Hospitals.hprofile.update') }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class = "grid grid-cols-2 gap-6">
                            <div class = "grid grid-rows-2 gap-6">
                                <div class = "form-group mt-4 mx-2">
                                    ...
                                    <x-text-input id="hospitalName" class="form-control" type="text" name="name" value="{{Auth::guard('Hospitals')->user()->hospitalName}}" autofocus readonly />
                                </div>

                                <!-- Email -->
                                <div class = "form-group mt-4 mx-2">
                                    <x-input-label for="email" :value="__('Email')" />
                                    <x-text-input id="email" class="form-control" type="text" name="email" value="{{Auth::guard('Hospitals')->user()->email}}" autofocus />
                                </div>

                                <!-- Hospital Website Link -->
                                <div class = "form-group mt-4 mx-2">
                                    <x-input-label for="hospitalLink" :value="__('Hospital Website Link')" />
                                    <x-text-input id="hospitalLink" class="form-control" type="text" name="hospitalLink" value="{{Auth::guard('Hospitals')->user()->hospitalLink}}" autofocus/>
                                </div>

                                <!-- Contact Number -->
                                <div class = "form-group mt-4 mx-2">
                                    <x-input-label for="phoneNo" :value="__('Contact Number')" />
                                    <x-text-input id="phoneNo" class="form-control" type="text" name="phoneNo" value="{{Auth::guard('Hospitals')->user()->phoneNo}}" autofocus />
                                </div>

                                <!-- Address -->
                                <div class = "form-group mt-4 mx-2">
                                    <x-input-label for="address" :value="__('Address')" />
                                    <x-text-input id="address" class="form-control" type="text" name="address" value="{{Auth::guard('Hospitals')->user()->address}}" autofocus />
                                </div>

                                <!-- Zip Code -->
                                <div class = "form-group mt-4 mx-2">
                                    <x-input-label for="zipCode" :value="__('Zip Code')" />
                                    <x-text-input id="zipCode" class="form-control" type="number" name="zipCode" value="{{Auth::guard('Hospitals')->user()->zipCode}}" autofocus />
                                </div>
